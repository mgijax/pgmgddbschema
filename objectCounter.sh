...
cd ${TOP}/key
echo "Primary Keys            `ls *_create.object | wc -l` scripts  (`grep -i 'PRIMARY KEY' *_create.object | wc -l` foreign keys)"
psql -h ${PG_DBSERVER} -U ${PG_DBUSER} -d ${PG_DBNAME} --command "select count(c.conname) from pg_constraint c join pg_namespace n on n.oid = c.connamespace where c.contype in ('p') and n.nspname = 'mgd'"

echo "Keys            `ls *_create.object | wc -l` scripts  (`grep -i '^ALTER TABLE' *_create.object | grep -i 'FOREIGN KEY' | wc -l` foreign keys)"
