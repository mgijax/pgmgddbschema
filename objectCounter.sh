...
#            the ones defined in the pgmgddbschema product to make sure
...
echo "Database:       ${PG_DBSERVER}.${PG_DBNAME}.mgd"
...
#        - remove the "mgd." prefix from the table name
...
# Triggers
#
# 1) Get a list of trigger names defined in the schema product.
#    - the trigger name is in field 3
# 2) Get a list of trigger names defined in the database.
# 3) Call the diff function.
#
cd ${TOP}/trigger
rm -f ${TMP_SCHEMA} ${TMP_DB}
...
# Views
#
# 1) Get a list of view names defined in the schema product.
#    - to derive the view name that should be in the database:
#        - use the table name in field 3
#        - remove the "mgd." prefix from the table name
# 2) Get a list of view names defined in the database.
# 3) Call the diff function.
#
cd ${TOP}/view
rm -f ${TMP_SCHEMA} ${TMP_DB}
...
cd ${TOP}
rm -f ${TMP_DB}
psql -h ${PG_DBSERVER} -d ${PG_DBNAME} -U ${PG_DBUSER} -t --command "select c.conname as duplidateKeys from pg_constraint c join pg_namespace n on n.oid = c.connamespace where c.contype in ('f') and n.nspname = 'mgd' and c.conname like '%fkey1%'" | grep -v "^$" | sort > ${TMP_DB}
echo ""
echo ""
